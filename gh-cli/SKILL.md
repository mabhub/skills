...
|---|---|---|
| **Lecture** | `repo view`, `issue list`, `issue view`, `pr list`, `pr view`, `pr diff`, `run list`, `run view`, `release list`, `status`, `search`, `api` (GET) | Aucune |
| **Écriture** | `issue create`, `issue edit`, `issue comment`, `pr create`, `pr review`, `pr comment`, `pr edit`, `pr ready`, `release create`, `label create`, `api` (POST) | Afficher le contenu complet, attendre confirmation |
| **Destructif** | `pr merge`, `pr close`, `issue close`, `repo delete`, `repo archive`, `release delete`, `label delete`, `api -X DELETE`, `api -X PUT` | Afficher la commande, expliquer l'impact, attendre confirmation avec rappel d'irréversibilité |

### Opérations interdites

Ne JAMAIS exécuter, même sur demande explicite :

- `gh repo delete` — suppression de dépôt
- `gh api -X DELETE /repos/{owner}/{repo}` — suppression via API
- `gh api -X PATCH /repos/{owner}/{repo}` avec `visibility: "public"` — rendre un dépôt public
- `gh api -X DELETE` sur `/repos/{owner}/{repo}/branches/protection` — supprimer les protections de branche
- `gh secret set` avec une valeur qui ressemble à un token ou clé privée en dur
- `gh repo fork` sans confirmation (crée un fork public)

Si l'utilisateur demande une opération interdite : refuser, expliquer le risque, et suggérer l'alternative via l'interface web GitHub.

### Garde-fous pour `gh api` (API brute)
...
- Ne JAMAIS afficher de token réel (`ghp_*`, `gho_*`, `ghs_*`, `github_pat_*`) dans la sortie ou les confirmations
- Utiliser `$GH_TOKEN` dans les commandes — jamais de valeur en dur
- Ne jamais afficher les valeurs des secrets (`gh secret list` n'expose pas les valeurs, mais `gh api` pourrait)
- Si la sortie contient un pattern `ghp_`, `gho_`, `ghs_`, ou `github_pat_` : tronquer et avertir
...
- Résultat (succès/échec + URL de la ressource créée/modifiée)
